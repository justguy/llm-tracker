...
### 15.1.5 Resolved in v0.7 (design integration)

- **Multi-task per session is a UI projection, not concurrent jobs.** A session has at most one `activeJobId` and an ordered `queuedJobIds: JobId[]`. The large card shows a `SessionTaskLedgerItem[]` view-model derived from JobRegistry + tracker queue + RunCandidateService. `SessionRecord.activeJobIds: array` is explicitly rejected. (TDD §23.2 #29.)
- **Model swap default = restart-as-successor.** `[Restart with new model]` creates a successor session/job using deterministic rollover context. Live mid-thread swap is gated by `providerCapabilities.modelSwapMidThread` and labeled distinctly in UI. Top-level `[SWAP MODEL]` requires an unambiguous selection scope. (TDD §23.2 #30, §25 `modelSwap`.)
- **Triage labels may be more specific than ActivityState.** UI chips `NO_HEARTBEAT`, `BLOCKED_ON_DEP`, `QUIET`, `APPROVAL`, `CTX_HIGH` are display-only; underlying ActivityStates remain `quiet`/`blocked`/`waiting_for_approval`/`context_high` with warning subtypes. No new ActivityStates. (TDD §23.2 #31.)
- **`+ Add Task` in Run Session writes durable tracker truth first.** Transient `NewTaskFormDraft` becomes a real tracker task before `RunSessionDraft.taskId` is populated. Runtime-only "draft tasks" launchable as jobs are explicitly rejected. (TDD §23.2 #32.)
- **Untasked sessions are allowed.** Global `[+ NEW SESSION]` enters the Run Session funnel with `mode: 'task_backed' | 'untasked' | 'attach_existing'`. Untasked mode creates `SessionRecord` without `JobRecord`, raises `unbound_session` attention, and disables job-only actions until `[Bind task]`. (TDD §23.2 #33, §25 `untaskedSessions`.)
- **Single launch funnel.** `SessionBrief` (a.k.a. RunSessionWizard) is the canonical launcher. The parallel `StartSessionWizard` 5-step modal is removed from the design source; CI guard prevents reintroduction. (TDD §23.2 #34.)
- **Multi-task attach = queue, not parallel execution.** Dragging a task onto an active session creates a queued successor job (`JobQueuedEvent`, `predecessorJobId` linked). A session never has more than one `running` JobRecord. The ledger view-model shows queued jobs as `relation: "queued_next"`. (TDD §23.2 #35.)

### 15.2 Still open

_(no remaining product-level open questions; see TASKS file §1.2 for advisory implementation gaps from the design-integration review.)_
