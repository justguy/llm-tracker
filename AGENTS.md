...
- for linked shared-workspace projects, `GET /api/projects/<slug>` and successful `POST /api/projects/<slug>/patch` responses include `file`, the effective tracker JSON path the hub writes; successful patch responses also include `applied`, the accepted post-write values for fields named in the patch
- for linked shared-workspace projects, `status`, `assignee`, `blocker_reason`, `meta.scratchpad`, `updatedAt`, and `rev` write through to the linked repo-local file; legacy `<shared-workspace>/.runtime/overlays/<slug>.json` files are migration artifacts, not project truth
- after a tracker write, use the patch response `applied` block and focused tracker reads (`tracker_brief`, `tracker_project_status`, `tracker_changed`, or `GET /api/projects/<slug>`) to verify accepted state; do not open the tracker JSON merely to check whether an update landed
...
- before push or merge, prove the branch contains the live tracker truth for the touched tasks with tracker responses plus git status/diff metadata; do not manually parse the JSON projection as the verification mechanism, and do not restore, stash, or discard tracker diffs; if merging would regress tracker truth, stop and make a tracker-sync commit/PR first
