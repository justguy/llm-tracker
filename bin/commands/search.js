...
function formatMatch(match, index) {
  ...
  if (match.assignee) extras.push(`assignee=${match.assignee}`);
...
export async function cmdSearch(args, { resolveWorkspace, httpRequest }) {
  ...
  if (!slug || !query) {
    console.error("Usage: llm-tracker search <slug> <query> [--json] [--limit N]");
    process.exit(1);
  }
  ...
  const { status, body } = await httpRequest(
    ...
    args.flags.port,
    "GET",
    `/api/projects/${slug}/search?q=${encodeURIComponent(query)}&limit=${limit}`
  );

  ensureHubResponse(status, body, "Search");
  ...
  console.log(`  ${body.project}  rev ${body.rev ?? "?"}  semantic search  generated ${body.generatedAt}`);
